...
/*
Copy and paste this code in https://www.codingame.com/ide/puzzle/the-fall-episode-3
*/
...
func MoveRocks(rs []Entity) []Entity {
	var newR []Entity
	var newNewR []Entity
	posCounter := make(map[[2]int]int)
	for _, r := range rs {
		rock := r.Move()
		if rock.Pos == "X" {
			continue
		}
		posCounter[rock.XY] += 1
		newR = append(newR, rock)
	}
	for _, r := range newR {
		if posCounter[r.XY] <= 1 {
			newNewR = append(newNewR, r)
		}
	}
	return newNewR
}
...
func SpawnRocks(cantSpawn map[[2]int]bool) []Entity {
	var rocks []Entity
	for j, v := range Grid[0] {
		...
		if cant := cantSpawn[[2]int{0, j}]; Room[r]["TOP"] != [2]int{0, 0} && !cant {
			rocks = append(rocks, Entity{"TOP", 0, j, [2]int{0, j}})
		}
	}
	return rocks
}
...
func Dps(Indy Entity, Rocks []Entity, state State) bool {
	...
	if state.Depth == 1 && Indy.X == 2 {
		newRocks = append(newRocks, SpawnRocks(cantRotate)...)
	}
